...
-- Use a temporary table approach to avoid duplicate key issues
DO $$
BEGIN
  -- Create a temporary table to aggregate the data first
  ...
  SELECT 
    ...
    s.unit_type, -- Use SKU's unit_type instead of vehicle arrival item's unit_type
  ...
  -- Now insert or update from the temporary table
