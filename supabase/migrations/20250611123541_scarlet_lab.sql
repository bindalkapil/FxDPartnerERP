/*
  ...
  3. Indexes
    - Add indexes for product_id, sku_id, product_name, and category

  4. Triggers
    - Add trigger to update `updated_at` timestamp

  5. Initial Data
    - Populate table with aggregated data from completed vehicle arrivals
*/
...
-- Use a CTE to properly aggregate data and avoid duplicate constraint violations
WITH aggregated_inventory AS (
  SELECT 
    ...
    p.category,
    vai.unit_type,
    COALESCE(SUM(vai.final_quantity), 0) as total_available_quantity,
    COALESCE(SUM(vai.final_total_weight), 0) as total_weight_sum,
  ...
  WHERE va.status IN ('completed', 'po-created')
    AND vai.final_quantity IS NOT NULL
    AND vai.final_total_weight IS NOT NULL
