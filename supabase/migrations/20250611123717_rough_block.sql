/*
  # Create current_inventory table and populate with vehicle arrival data
  ...
  3. Changes
    ...
    - Handle duplicate product-sku combinations by taking the first unit_type found
*/
...
-- Initialize/update current_inventory with data from existing vehicle arrivals
-- Use DISTINCT ON to ensure unique (product_id, sku_id) combinations
WITH ranked_inventory AS (
  SELECT 
    ...
    ROW_NUMBER() OVER (
      ...
      ORDER BY MAX(va.arrival_time) DESC, vai.unit_type
    ) as rn
